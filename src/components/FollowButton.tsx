...
export function followLabel(iFollow: boolean, followsMe: boolean): string {
  if (iFollow) return 'Вы подписаны';
  return followsMe ? 'Подписаться в ответ' : 'Подписаться';
}
...
export default function FollowButton({ iFollow, followsMe, busy = false, compact = false, onPress }: FollowButtonProps) {
  ...
  const label = followLabel(iFollow, followsMe);
  return (
    <Pressable
      onPress={onPress}
      disabled={busy}
      ...
      accessibilityLabel={iFollow ? 'Отписаться' : label}
    >
      {busy ? (
